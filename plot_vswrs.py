...
vswr_main_files_str = sys.argv[4]  # eg. 'vswr-main-files.json' - must be located in plot_location.
...
def main():
    ...
    for j in range(len(all_files)):
        for ant, v in all_files[j].iteritems():
    ...
    for ant, dataset in all_data.items():
        ...
        for entry in dataset:
            ...
            # best_fit.append(best_fit_value)
    ...
    for ant, dataset in all_data.items():
        max_phase_offset = 0
        for index, entry in enumerate(dataset):
            phase_offset = entry['phase'] - phase_ave[index]
    ...
    fig, smpplot = plt.subplots(5, sharex=True, figsize=(16,22))
    ...
    for ant, dataset in all_data.items():
        ...
        smpplot[4].plot(dataset['freq'], linear_fit_dict[ant]['offset_of_best_fit'], label='{}, stderr={}'.format(ant,round(linear_fit_dict[ant]['stderr'], 9)), color=hex_dictionary[ant])
    smpplot[2].plot(all_data['M0']['freq'], diff_phase, label='Max-Min Difference', color=hex_dictionary['other'])
    smpplot[3].plot(all_data['M0']['freq'], swr_ave, label='Average SWR', color=hex_dictionary['other'])
    for antenna in worst_swrs:
        smpplot[3].plot(all_data[antenna]['freq'], all_data[antenna]['VSWR'], label=antenna, color=hex_dictionary[antenna])
        smpplot[2].plot(all_data[antenna]['freq'], all_data[antenna]['phase'] - phase_ave, label=antenna, color=hex_dictionary[antenna])
