...
def retrieve_data_from_csv(map_to_files, data_location, header_names):
    ...
    for k, v in vswr_files.items():
        ...
        with open(data_location + v, 'r') as csvfile:
            ...
            # All keys should start with M or I to indicate which array they
            # are from.
            if k[0] == 'M' or k[0] == 'I':
                all_data[k] = rawdata
            else:
...
def create_lmr400_cable_loss_array(ref_freq_list, cable_length):
    ...
    cable_loss = [cable_length / 100.0 * (0.122290 * math.sqrt(freq * 1.0e-6) + 0.000260 * freq * 1.0e-6)
                  for freq in ref_freq_list]

    cable_loss_array = []
    for freq, loss in zip(ref_freq_list, cable_loss):
        cable_loss_array.append((freq, loss))
    cable_loss_array = np.array(
        cable_loss_array, dtype=[
            ('freq', 'i4'), ('loss', 'f4')])
...
def create_belden_8214_cable_loss_array(ref_freq_list, cable_length):
    ...
    cable_loss = [cable_length / 100.0 * 10.0 ** (slope_1 * math.log(freq * 1.0e-6, 10.0) + intercept)
                  for freq in ref_freq_list if freq <= 10000000]
    cable_loss_2 = [cable_length / 100.0 * 10.0**(slope_2 * math.log(freq * 1.0e-6, 10.0) + intercept)
                    for freq in ref_freq_list if freq > 10000000]
...
def create_belden_9913_cable_loss_array(ref_freq_list, cable_length):
    ...
    # convert to metres because these attenuation
    cable_length /= 3.2808
...
def create_carol_c1180_cable_loss_array(ref_freq_list, cable_length):
    ...
    slope_1 = math.log(ref_10_mhz / ref_1_mhz, 10.0) / \
        math.log(10.0 / 1.0, 10.0)
    slope_2 = math.log(ref_50_mhz / ref_10_mhz, 10.0) / \
        math.log(50.0 / 10.0, 10.0)
...
def create_eupen_ec400_cable_loss_array(ref_freq_list, cable_length):
    ...
    slope_1 = math.log(ref_20_mhz / ref_10_mhz, 10.0) / \
        math.log(20.0 / 10.0, 10.0)
    slope_2 = math.log(ref_30_mhz / ref_20_mhz, 10.0) / \
        math.log(30.0 / 20.0, 10.0)
