...
# eg. 'vswr-files.json' - must be located in plot_location.
vswr_files_str = sys.argv[4]
# vswr_intf_files_str = sys.argv[5]  # eg. 'vswr-intf-files.json' - must
# be location in plot_location.
...
def main():
        ...
        with open(data_location + v, 'r') as csvfile:
            for line in csvfile:
                ...
                if fnmatch.fnmatch(line,
                                   'Freq [Hz*') or fnmatch.fnmatch(line,
                                                                   'Frequency [Hz*'):
                    break
            else:  # no break
    ...
    for ant, dataset in all_data_phase_wrapped.items():
        # wrapping then unwrapping allows us to
    ...
    for ant, dataset in all_data.items():
        ...
        if ant == 'M0':
            ...
            swr_ave = [swr_ave[i] + dataset['VSWR'][i]
                       for i in range(len(dataset))]
            phase_ave = [phase_ave[i] + dataset['phase'][i]
                         for i in range(len(dataset))]
    ...
    for ant, dataset in all_data.items():
        ...
        offset_of_best_fit = np.array(
            offset_of_best_fit, dtype=[
                ('phase', 'f4')])
        ...
        linear_fit_dict[ant]['best_fit_line'] = wrap_phase(
            linear_fit_dict[ant]['best_fit_line'])
    ...
    while len(worst_swrs) < 5:
        worst_swrs.append(max(furthest_phase_offset,
                              key=lambda key: furthest_phase_offset[key]))
    ...
    for antenna in worst_swrs:
        ...
        worst_swrs_phase_offset[antenna] = np.array(
            phase_offset_list, dtype=[('phase', 'f4')])
        worst_swrs_phase_offset[antenna] = wrap_phase(
            worst_swrs_phase_offset[antenna])

    numplots = 6
    fig, smpplot = plt.subplots(
        numplots, sharex=True, figsize=(
            16, 22), dpi=80)
    ...
    for antenna in worst_swrs:
        smpplot[3].plot(
            all_data[antenna]['freq'],
            all_data[antenna]['VSWR'],
            label=antenna,
