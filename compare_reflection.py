...
def main():
        ...
        with open(data_location + v, 'r') as csvfile:
            ...
            try:
                ...
                if (abs(mag_column - freq_column) > 2) or (
                    ...
                    sys.exit('Data Phase and Magnitude are given from different sweeps - please'
    ...
    cable_distance = 22.1
    slope_15m = -360.0 * cable_distance / (constants.speed_of_light * velocity_factor)
    ...
    for ant, dataset in after_data.items():
        ...
        for num, freq in enumerate(dataset['freq']):
            data.append((freq, (dataset['phase'][num] - ((freq - starting_freq) * slope_15m * 2 + starting_phase))))
        after_data_with_15m_removed[ant] = np.array(data, dtype=[('freq', 'i4'), ('phase', 'f4')])
    ...
    for ant, dataset in after_data_with_15m_removed.items():
        smpplot[1].plot(dataset['freq'], dataset['phase'], label=ant, color=hex_dictionary[ant])
    for ant, dataset in after_data.items():
        smpplot[2].plot(dataset['freq'], dataset['dB'], label=ant, color=hex_dictionary[ant])
    ...
    after_data_with_15m_removed_wrapped = wrap_phase_dictionary(after_data_with_15m_removed)


    for ant, dataset in before_data.items():
        fig, newplot = plt.subplots()
        newplot.plot(before_fixes_data[ant]['freq'], before_fixes_data[ant]['phase'], label='2009 before fixes', color='m')
        newplot.plot(dataset['freq'], dataset['phase'], label='2009 after fixes', color='b')
        newplot.plot(after_data[ant]['freq'], after_data[ant]['phase'], label='20171212', color='r')
        newplot.plot(after_data_with_15m_removed[ant]['freq'], after_data_with_15m_removed[ant]['phase'], label='20171212, cable removed', color='g')
        newplot.grid()
        newplot.legend()
        ...
        #wrapped_plot.plot(before_fixes_data_wrapped[ant]['freq'], before_fixes_data_wrapped[ant]['phase'], label='2009 before fixes', color='m')
        wrapped_plot.plot(before_data_wrapped[ant]['freq'], before_data_wrapped[ant]['phase'], label='2009 after fixes', color='b')
        wrapped_plot.plot(after_data_with_15m_removed_wrapped[ant]['freq'], after_data_with_15m_removed_wrapped[ant]['phase'], label='20171212, cable removed', color='g')
        wrapped_plot.grid()
        wrapped_plot.legend()
